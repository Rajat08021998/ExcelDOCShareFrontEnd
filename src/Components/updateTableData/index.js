...
const UpdateTableData = (props) => {
  ...
  console.log("================>", rowData);
  // Use the rowData object to edit the data

  const history = useHistory();
  const socket = useContext(SocketContext);
  const [fileName, setFileName] = useState(
    rowData.dataObj && rowData.dataObj.length > 0 ? rowData.dataObj : []
  );
  const [docName, setDocName] = useState({});
  const [headings, setHeadings] = useState(
    rowData.headingObj && rowData.headingObj.length > 0
      ? rowData.headingObj
      : []
  );
  ...
  const [id, setId] = useState(
    rowData.obj && rowData.obj._id ? rowData.obj._id : null
  );
  useEffect(() => {
    let unmounted = false;
    if (!unmounted) {
      setDocName({
        name: rowData.obj && rowData.obj.DOC_NAME ? rowData.obj.DOC_NAME : "",
      });
      let data = [...fileName];
  ...
  const handleUpdateDoc = () => {
    const body = {
      ...
      DOC_OBJ: fileName,
      DOC_LABEL: headings,
      UPDATED_BY: rowData.user,
    };
    let receiverArr = [];
    if (rowData.action) {
      if (rowData.action === "MAIN") {
        let receivers = [...rowData.obj.EDIT_LIST, ...rowData.obj.VIEW_LIST];
        receiverArr = [...new Set(receivers)];
      } else if (rowData.action === "EDIT") {
        let receivers = [
          ...rowData.obj.EDIT_LIST,
          ...rowData.obj.VIEW_LIST,
          rowData.obj.CREATED_BY,
        ];
        receiverArr = [...new Set(receivers)];
        receiverArr = receiverArr.filter(function (item) {
          return item !== rowData.user;
        });
      }
    }
    if (
  ...
  return (
    <>
                ...
                >
                  Save <i className="fa fa-download"></i>
                    ...
                    <div style={{ display: "flex" }}>
                      ...
                      {!rowData.type || rowData.type !== "view" ? (
                        <>
                          <IconButton
                            aria-label="Save Doc"
      ...
      {showEnterData && (!rowData.type || rowData.type !== "view") ? (
        <FormDialog
          headings={headings}
          fileName={fileName}
