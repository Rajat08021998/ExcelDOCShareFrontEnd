...
import { ReactComponent as EditDocIcon } from "./../../images/editDoc.svg";
...
export default function SharedUserEdit(props) {
  ...
  const getData = (user) => {
    axios
      .get(`${apiUrl}/doc/edit/${user.userId}`, {})
      .then((res) => res.data)
      .then((res) => {
  ...
  return (
    <>
      {!editDoc ? (
        <Box sx={{ flexGrow: 1 }}>
          {/* <h2>List of Doc For View/Edit</h2> */}
          <div style={{ padding: "10px" }}>
            <TabsHeading text={"Edit DOC List"} icon={EditDocIcon} />
          </div>
