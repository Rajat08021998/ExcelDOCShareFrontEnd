...
export default function ViewDocList(props) {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.status === "success" && res.result) {
          ...
          const deletedDoc = orginalData.filter((item) => item._id === id);
          ...
          // console.log("Data Added Unsucessfull");
          alert("Data Deleted Unsucessfull");
        }
      })
      .catch((err) => {
        alert("Something went wrong" + err);
        console.log("Data Deleted Unsucessfull");
        setLoading(false);
      });
  };
      ...
      {!editDoc ? (
        <Box sx={{ flexGrow: 1 }}>
          ...
          <div style={{ padding: "10px" }}>
            <TabsHeading text={"DOC List"} icon={FolderIcon} />
          </div>
